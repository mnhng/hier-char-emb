...
def evaluate(args, model, data_source, criterion, batch_size=10):
...
def train(args, model, train_data, optimizer, criterion, params, epoch):
...
def main(args):
    ...
    emb_layer = torch.nn.Embedding(ntokens, args.emsize)
    emb_layer.weight.data.uniform_(-.1, .1)
    ...
    try:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            train(args, model, train_data, optimizer, criterion, params, epoch)
            if 't0' in optimizer.param_groups[0]:
                ...
                with torch.no_grad():
                    val_loss2 = evaluate(args, model, val_data, criterion)
                print(
                ...
                with torch.no_grad():
                    val_loss = evaluate(args, model, val_data, criterion, eval_batch_size)
                ...
                if epoch in args.when:
                    ...
                    torch.save([model, criterion, optimizer], f'{args.save}.e{epoch}')
    ...
    # Run on test data.
    with torch.no_grad():
        test_loss = evaluate(args, model, test_data, criterion, test_batch_size)
    print(
