...
def evaluate(args, model, data_source, batch_size=10):
    ...
    for i in range(0, data_source.size(0) - 1, args.bptt):
        ...
        total_loss += len(data) * F.nll_loss(F.log_softmax(model.decoder(output), dim=-1), targets)
        hidden = repackage_hidden(hidden)
    return total_loss.item() / len(data_source)


def train(args, model, train_data, optimizer, params, epoch):
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        loss = F.nll_loss(F.log_softmax(model.decoder(output), dim=-1), targets)
...
def main(args):
    ...
    emb_layer = HierarchicalEmbedding(len(corpus.dictionary), args.emsize, corpus.dictionary.word2idx)
    model = RNNModel(args.model, emb_layer, args.nhid, args.nlayers,
    ...
    try:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            train(args, model, train_data, optimizer, params, epoch)
            if 't0' in optimizer.param_groups[0]:
                ...
                with torch.no_grad():
                    val_loss2 = evaluate(args, model, val_data)
                print(
                ...
                if val_loss2 < stored_loss:
                    torch.save([model, optimizer], args.save)
                ...
                with torch.no_grad():
                    val_loss = evaluate(args, model, val_data, eval_batch_size)
                val_ppl = math.exp(val_loss)
                val_bpc = val_loss / math.log(2)
                print(
                    '| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.2f} | '
                    'valid ppl {:8.2f} | valid bpc {:8.3f}'.format(
                        epoch, (time.time() - epoch_start_time), val_loss, val_ppl, val_bpc))
                ...
                if epoch in args.when:
                    ...
                    torch.save([model, optimizer], f'{args.save}.e{epoch}')
    ...
    # Run on test data.
    with torch.no_grad():
        test_loss = evaluate(args, model, test_data, test_batch_size)
    print(
